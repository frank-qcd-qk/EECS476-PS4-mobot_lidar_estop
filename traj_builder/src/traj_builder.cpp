...
void TrajBuilder::build_braking_traj(geometry_msgs::PoseStamped start_pose,
        std::vector<nav_msgs::Odometry> &vec_of_states) {
    ROS_WARN("E-stop! E-stop! E-stop! E-stop! E-stop!");
    double x_des = start_pose.pose.position.x;
    ...
    double time_to_slowdown = speed_des / accel_max_;
    int command_points = (int) (time_to_slowdown / dt_);
        vec_of_states.clear();
        vec_of_states.push_back(des_state);
    for (int i = 0; i < command_points; i++) {
        speed_des -= accel_max_*dt_; //Euler one-step integration
        des_state.twist.twist.linear.x = speed_des;
